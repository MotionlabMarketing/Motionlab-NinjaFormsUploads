...
class NF_FU_External_Services_Googledrive_Service extends NF_FU_External_Abstracts_Oauthservice {
	...
	protected function drive() {
		if ( is_null( $this->drive ) ) {
			$this->drive = new Google_Service_Drive( $this->get_client() );
		}
	...
	protected function upload_file( $data ) {
		$args = array(
			'name' => $this->external_filename,
		);

		if ( ! empty ( $this->external_path ) && '/' !== $this->external_path ) {
			// Check folder exists or create it
			$folder_id = $this->create_or_get_directory( $this->external_path );

			$args['parents'] = array( $folder_id );
		}

		$fileMetadata = new Google_Service_Drive_DriveFile( $args );
