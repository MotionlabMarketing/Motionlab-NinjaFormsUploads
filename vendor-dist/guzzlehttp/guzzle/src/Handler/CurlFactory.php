...
class CurlFactory implements \NF_FU_VENDOR\GuzzleHttp\Handler\CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(\NF_FU_VENDOR\GuzzleHttp\Handler\EasyHandle $easy, array &$conf)
    {
        ...
        if (isset($options['sink'])) {
            $sink = $options['sink'];
            if (!\is_string($sink)) {
                $sink = \NF_FU_VENDOR\GuzzleHttp\Psr7\stream_for($sink);
        ...
        if (isset($options['proxy'])) {
            if (!\is_array($options['proxy'])) {
                ...
                if (isset($options['proxy'][$scheme])) {
                    ...
                    if (!isset($options['proxy']['no']) || !\NF_FU_VENDOR\GuzzleHttp\is_host_in_noproxy($host, $options['proxy']['no'])) {
        ...
        if (!empty($options['debug'])) {
            $conf[\CURLOPT_STDERR] = \NF_FU_VENDOR\GuzzleHttp\debug_resource($options['debug']);
