...
use NF_FU_VENDOR\GuzzleHttp\HandlerStack;
...
class MockHandler implements \Countable
{
    ...
    public function __invoke(\NF_FU_VENDOR\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        $response = $response instanceof \Exception ? \NF_FU_VENDOR\GuzzleHttp\Promise\rejection_for($response) : \NF_FU_VENDOR\GuzzleHttp\Promise\promise_for($response);
        return $response->then(function ($value) use($request, $options) {
            ...
            if (isset($options['sink'])) {
                ...
                } elseif ($sink instanceof \NF_FU_VENDOR\Psr\Http\Message\StreamInterface) {
                    $sink->write($contents);
                }
            }
            return $value;
