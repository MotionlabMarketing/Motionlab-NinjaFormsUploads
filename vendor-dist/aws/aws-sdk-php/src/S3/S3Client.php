...
use NF_FU_VENDOR\Aws\Api\ApiProvider;
use NF_FU_VENDOR\Aws\Api\DocModel;
use NF_FU_VENDOR\Aws\Api\Service;
use NF_FU_VENDOR\Aws\AwsClient;
use NF_FU_VENDOR\Aws\CacheInterface;
use NF_FU_VENDOR\Aws\ClientResolver;
use NF_FU_VENDOR\Aws\Command;
use NF_FU_VENDOR\Aws\Exception\AwsException;
use NF_FU_VENDOR\Aws\HandlerList;
use NF_FU_VENDOR\Aws\Middleware;
use NF_FU_VENDOR\Aws\RetryMiddleware;
use NF_FU_VENDOR\Aws\ResultInterface;
use NF_FU_VENDOR\Aws\CommandInterface;
use NF_FU_VENDOR\Aws\S3\RegionalEndpoint\ConfigurationProvider;
...
class S3Client extends \NF_FU_VENDOR\Aws\AwsClient implements \NF_FU_VENDOR\Aws\S3\S3ClientInterface
{
    ...
    public function createPresignedRequest(\NF_FU_VENDOR\Aws\CommandInterface $command, $expires, array $options = [])
    {
        ...
        return $signer->presign(\NF_FU_VENDOR\Aws\serialize($command), $this->getCredentials()->wait(), $expires, $options);
    }
    /**
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \NF_FU_VENDOR\Aws\serialize($command)->getUri();
    }
    /**
