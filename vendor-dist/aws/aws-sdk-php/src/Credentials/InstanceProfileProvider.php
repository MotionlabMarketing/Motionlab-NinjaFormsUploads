...
use NF_FU_VENDOR\Aws\Exception\InvalidJsonException;
...
class InstanceProfileProvider
{
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        $userAgent .= ' ' . \NF_FU_VENDOR\Aws\default_user_agent();
        ...
        return $fn($request, ['timeout' => $this->timeout])->then(function (\NF_FU_VENDOR\Psr\Http\Message\ResponseInterface $response) {
            ...
            if ($reason instanceof \NF_FU_VENDOR\GuzzleHttp\Exception\RequestException) {
                throw $reason;
            }
