...
use NF_FU_VENDOR\Monolog\Formatter\LogglyFormatter;
...
class LogglyHandler extends \NF_FU_VENDOR\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $token, $level = \NF_FU_VENDOR\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('curl')) {
            throw new \NF_FU_VENDOR\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \NF_FU_VENDOR\Monolog\Handler\Curl\Util::execute($ch, 5, \false);
    }
