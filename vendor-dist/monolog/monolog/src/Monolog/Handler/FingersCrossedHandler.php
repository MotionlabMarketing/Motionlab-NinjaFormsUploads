...
use NF_FU_VENDOR\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use NF_FU_VENDOR\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
 ...
 */
class FingersCrossedHandler extends \NF_FU_VENDOR\Monolog\Handler\Handler implements \NF_FU_VENDOR\Monolog\Handler\ProcessableHandlerInterface, \NF_FU_VENDOR\Monolog\ResettableInterface, \NF_FU_VENDOR\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = \true, bool $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \NF_FU_VENDOR\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\NF_FU_VENDOR\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \NF_FU_VENDOR\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \NF_FU_VENDOR\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \NF_FU_VENDOR\Monolog\Logger::toMonologLevel($passthruLevel);
        }
    ...
    public function reset()
    {
        ...
        if ($this->getHandler() instanceof \NF_FU_VENDOR\Monolog\ResettableInterface) {
