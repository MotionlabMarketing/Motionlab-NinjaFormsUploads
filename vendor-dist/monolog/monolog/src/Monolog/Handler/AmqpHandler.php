...
class AmqpHandler extends \NF_FU_VENDOR\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct($exchange, ?string $exchangeName = null, $level = \NF_FU_VENDOR\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if ($exchange instanceof \NF_FU_VENDOR\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    private function createAmqpMessage(string $data) : \NF_FU_VENDOR\PhpAmqpLib\Message\AMQPMessage
    {
        return new \NF_FU_VENDOR\PhpAmqpLib\Message\AMQPMessage($data, ['delivery_mode' => 2, 'content_type' => 'application/json']);
    }
    /**
     * {@inheritDoc}
     */
