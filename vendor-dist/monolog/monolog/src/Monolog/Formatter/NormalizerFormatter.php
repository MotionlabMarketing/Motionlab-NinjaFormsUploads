 ...
 */
class NormalizerFormatter implements \NF_FU_VENDOR\Monolog\Formatter\FormatterInterface
{
    public const SIMPLE_DATE = "Y-m-d\\TH:i:sP";
    protected $dateFormat;
    protected $maxNormalizeDepth = 9;
    protected $maxNormalizeItemCount = 1000;
    private $jsonEncodeOptions = \NF_FU_VENDOR\Monolog\Utils::DEFAULT_JSON_FLAGS;
    /**
    ...
    protected function normalize($data, int $depth = 0)
    {
        ...
        if (\is_object($data)) {
            ...
            return [\NF_FU_VENDOR\Monolog\Utils::getClass($data) => $value];
        }
        if (\is_resource($data)) {
    ...
    protected function normalizeException(\Throwable $e, int $depth = 0)
    {
        ...
        $data = ['class' => \NF_FU_VENDOR\Monolog\Utils::getClass($e), 'message' => $e->getMessage(), 'code' => (int) $e->getCode(), 'file' => $e->getFile() . ':' . $e->getLine()];
    ...
    {
        return \NF_FU_VENDOR\Monolog\Utils::jsonEncode($data, $this->jsonEncodeOptions, $ignoreErrors);
    }
    protected function formatDate(\DateTimeInterface $date)
    {
        // in case the date format isn't custom then we defer to the custom DateTimeImmutable
        // formatting logic, which will pick the right format based on whether useMicroseconds is on
        if ($this->dateFormat === self::SIMPLE_DATE && $date instanceof \NF_FU_VENDOR\Monolog\DateTimeImmutable) {
            return (string) $date;
        }
