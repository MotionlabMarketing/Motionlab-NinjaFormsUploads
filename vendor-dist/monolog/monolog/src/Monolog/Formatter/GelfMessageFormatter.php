...
class GelfMessageFormatter extends \NF_FU_VENDOR\Monolog\Formatter\NormalizerFormatter
{
     ...
     * {@inheritdoc}
     */
    public function format(array $record) : \NF_FU_VENDOR\Gelf\Message
    {
        ...
        if ($len > $this->maxLength) {
            $message->setShortMessage(\NF_FU_VENDOR\Monolog\Utils::substr($record['message'], 0, $this->maxLength));
        }
        ...
        foreach ($record['extra'] as $key => $val) {
            ...
            if ($len > $this->maxLength) {
                $message->setAdditional($this->extraPrefix . $key, \NF_FU_VENDOR\Monolog\Utils::substr($val, 0, $this->maxLength));
                continue;
            }
        ...
        foreach ($record['context'] as $key => $val) {
            ...
            if ($len > $this->maxLength) {
                $message->setAdditional($this->contextPrefix . $key, \NF_FU_VENDOR\Monolog\Utils::substr($val, 0, $this->maxLength));
                continue;
            }
