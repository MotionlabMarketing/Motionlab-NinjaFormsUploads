...
use DateTimeZone;
use NF_FU_VENDOR\Monolog\Handler\HandlerInterface;
...
class Logger implements \NF_FU_VENDOR\Psr\Log\LoggerInterface, \NF_FU_VENDOR\Monolog\ResettableInterface
{
     ...
     */
    public function pushHandler(\NF_FU_VENDOR\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \NF_FU_VENDOR\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
    ...
    public function addRecord(int $level, string $message, array $context = []) : bool
    {
        ...
        $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => new \NF_FU_VENDOR\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
        try {
    ...
    public static function toMonologLevel($level) : int
    {
        ...
        if (!\is_int($level)) {
            throw new \NF_FU_VENDOR\Psr\Log\InvalidArgumentException('Level "' . \var_export($level, \true) . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
        return $level;
    }
    /**
